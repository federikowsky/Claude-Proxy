...
* Classify each tool-use block using :class:`~claude_proxy.application.runtime_actions.RuntimeActionClassifier`.
* Apply the per-model capability policy for each action category:
  ...
  - ``BLOCK``   → raise :class:`~claude_proxy.domain.errors.RuntimeContractError`.
...
from claude_proxy.domain.enums import ActionPolicy, RuntimeActionType
...
_logger = logging.getLogger("claude_proxy.contract")
...
class RuntimeContractEnforcer:
    ...
    ----------
    action_classifier:
        The :class:`~claude_proxy.application.runtime_actions.RuntimeActionClassifier`
